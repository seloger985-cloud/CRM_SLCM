...
| **test-covers** | Une URL d’image entre désormais dans du HTML généré, et le champ `images` vient d’une base que ce dépôt ne contrôle pas : un champ absent, nul ou mal typé ne doit pas vider l’écran Propriétés, et un nom de fichier contenant un guillemet ne doit pas ouvrir de balise. Le contrôle sur `site.js` garde la colonne `images` dans la requête — sans elle les vignettes seraient muettes, **sans aucune erreur**. |
